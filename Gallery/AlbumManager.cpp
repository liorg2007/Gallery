...
void AlbumManager::interrupt()
{
	check_again:

	//check if queue is empty
	if (_externalApps.empty())
		exit();//regular exit

	//close the first picture that was opened
	PROCESS_INFORMATION pi = _externalApps.front();
...
{
	uint8_t option = 0;
	std::array<std::string, PICTURE_APP_COUNT> apps{"mspaint.exe", "i_view64.exe"};
	refreshOpenAlbum();
	...
	std::cout << "Select app to open the image with: " << "\n";
	std::cout << "1.Microsoft Paint\n" << "2.Irfanview\n";
	option = std::stoi(getInputFromConsole("Select option: "));

	while (option > PICTURE_APP_COUNT || option < 1)
	{
	...
	//show the picture
	openPicture(pic.getPath(), apps.at(option - 1));
}
...
void AlbumManager::removeUser()
{
	...
	//delete all user's tags in pictures
	//search in each album, in each album search in each picture. If a user is tagged in a picture then delete the tag
...
void AlbumManager::openPicture(const std::string& imagePath,const std::string& application)
{
	STARTUPINFO si;
	PROCESS_INFORMATION pi;

	std::string cmd = application + " " + imagePath;
	...
	{
		std::cout << "!!! Make sure the app selected exists on the computer and in the PATH !!!" << std::endl;
	}
	else
	{
		//push the process information(handles) to the queue
		_externalApps.push(pi);
	}
}
